...
static	int	fcomplex_type	= -1; // float complex
static	int	lcomplex_type	= -1; // long double complex

//
// Function for each variant to copy from va_list to mem
//
static	void	lcomplex_va_arg_function (void* mem, va_list* ap) {
	long double complex	z	= va_arg(*ap, long double complex);
	*(long double complex*) mem	= z;
}
static	void	dcomplex_va_arg_function (void* mem, va_list* ap) {
	double complex	z	= va_arg(*ap, double complex);
	*(double complex*) mem	= z;
}
static	void	fcomplex_va_arg_function (void* mem, va_list* ap) {
	float complex	z	= va_arg(*ap, float complex);
	*(float complex*) mem	= z;
}
                      ...
                      int* argtypes, int* size)
{
	if (n == 1) {
		if (info->is_long_double) {		
			...
			size [0]	= sizeof (long double complex);
		}
		else if (info->is_short) {
			argtypes [0]	= fcomplex_type;
			size [0]	= sizeof (float complex);
		}
		else {
			argtypes [0]	= dcomplex_type;
			size [0]	= sizeof (double complex);
		}
	}
	return 1;
}
...
//
	...
	if (len == -1)
		return	len;
	if (info->is_long_double) {
		long double complex	z = ***(long double complex***)(args);
		len	= snprintf (buffer, sizeof(buffer), fmt, creall (z), cimagl (z));
	}
	else if (info->is_short) {
		float complex	z	= ***(float complex***)(args);
		len	= snprintf (buffer, sizeof(buffer), fmt, crealf (z), cimagf (z));
	}
	else	{
		double complex	z	= ***(double complex***)(args);
